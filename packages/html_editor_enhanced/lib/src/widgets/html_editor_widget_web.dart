...
class _HtmlEditorWidgetWebState extends State<HtmlEditorWidget> {
  ...
  /// postMessage event.data can be String (JSON) or already Map in some browsers.
  static Map<String, dynamic> _parseMessageData(dynamic eventData) {
    if (eventData == null) return <String, dynamic>{};
    if (eventData is Map) return Map<String, dynamic>.from(eventData as Map);
    if (eventData is String) {
      try {
        return Map<String, dynamic>.from(json.decode(eventData) as Map);
      } catch (_) {
  ...
  /// Callbacks expect String; ensure contents/value from message is String.
  static String? _toMessageString(dynamic v) {
        ...
        if (p.onSelect != null) {
          html.window.onMessage.listen((event) {
            ...
            if (data['type'] != null &&
              ...
              p.onSelect!.call(_toMessageString(data['value']) ?? '');
            }
          });
        }
      }
        ...
        html.window.onMessage.listen((event) {
          ...
          if (data['type'] != null &&
            ...
            if (widget.callbacks != null &&
              ...
              widget.callbacks!.onChangeContent!.call(_toMessageString(data['contents']));
            }
    ...
    html.window.onMessage.listen((event) {
      ...
      if (data['type'] != null &&
        ...
        if (data['type'].contains('onBeforeCommand')) {
          c.onBeforeCommand!.call(_toMessageString(data['contents']));
        }
        if (data['type'].contains('onChangeContent')) {
          c.onChangeContent!.call(_toMessageString(data['contents']));
        }
        if (data['type'].contains('onChangeCodeview')) {
          c.onChangeCodeview!.call(_toMessageString(data['contents']));
        }
